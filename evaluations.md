# RnX_EVAL
![image](https://user-images.githubusercontent.com/81907010/218140447-4588efdd-fad0-4966-9645-ae8f1666bdb2.png)
<p align="center">
   Vilone et Longo, Information Fusion, 2021
</p>

## Objective evaluation -> metrics (Vilone et al., Information Infusion, 2021)
- **Infidelity** : the expected difference between the dot product of a significant input perturbation to the explanation and the subsequent output perturbation. (Yeh et al., On the (In)fidelity and Sensitivity of Explanations, NIPS, 2019)
- **Sensitivity** : measures the degree to which a visual explanation is affected by insignificant perturbations in the input instances. (Yeh et al., On the (In)fidelity and Sensitivity of Explanations, NIPS, 2019)
- **Filter interpretability** : considers six types of semantics for CNN filters that must be annotated by humans on testing images at the pixel level: objects, parts, scenes, textures, materials, and colours. The metric measures the intersection areas between these annotations and the distribution of the activation values of a filter over a heat-map. (Zhang et Zhu, Visual interpretability for deep learning: a survey, Frontiers of Information Technology & Electronic Engineering, 2018)
- **Spearman’s rank order correlation** : measure of rank correlation, i.e., statistical dependence between the rankings of two variables. It assesses how well the relationship between two variables can be described using a monotonic function. (Ghorbani et al., Interpretation of neural networks is fragile, NIPS Workshop on Machine Deception, 2017)
...
- Explanation completeness : check which method generates explanations that describe the inferential process of the underlying model to the highest extent. This consists of capturing the highest number of features of the input that affect the decision process of the model
...
<table>
  ...
  <tr>
    ...
    <td class="axioms"></td>
    <td class="ref">Yeh et al., On the (In)fidelity and Sensitivity of Explanations, NIPS (2019) </td>
  </tr>
  
  <tr>
    <td class="name">Completeness</td>
    <td class="exp"> FI + CB </td>
    <td class="prob"></td>
